...
! !PRIVATE MEMBER FUNCTIONS:
  ...
  private :: init_gridcell_hflux_type
!EOP
!----------------------------------------------------
...
!------------------------------------------------------------------------
    ...
    call init_gridcell_wflux_type(begg, endg, clm3%g%gwf)
     ...
     ! gridcell: heat flux variables

    call init_gridcell_hflux_type(begg, endg, clm3%g%ghf)
...
!------------------------------------------------------------------------
    ...
    allocate(gwf%qflx_qsurfg(beg:end)) !added by Hongyi
    allocate(gwf%qflx_qdraig(beg:end)) !added by Hongyi
    ...
    gwf%qflx_qsurfg(beg:end) = nan
    gwf%qflx_qdraig(beg:end) = nan
...
!BOP
!
! !IROUTINE: init_gridcell_hflux_type
!
! !INTERFACE:
  subroutine init_gridcell_hflux_type(beg, end, ghf)
!
! !DESCRIPTION:
! Initialize gridcell heat flux variables
!
! !USES:
    use clm_varcon, only : spval
! !ARGUMENTS:
    implicit none
    integer, intent(in) :: beg, end
    type (gridcell_hflux_type), intent(inout):: ghf
!
! !REVISION HISTORY:
! Created by Mariana Vertenstein
!
...
!------------------------------------------------------------------------
    ...
    allocate(ghf%t_grndg(beg:end)) !added by Hongyi
    allocate(ghf%zwtg(beg:end)) !added by Hongyi
    allocate(ghf%t_soisnog(beg:end,-nlevsno+1:nlevgrnd)) !added by Hongyi

    ghf%t_grndg(beg:end) = nan
    ghf%zwtg(beg:end) = nan
    ghf%t_soisnog(beg:end,:) = nan !spval

  end subroutine init_gridcell_hflux_type
