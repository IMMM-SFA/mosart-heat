...
!
    ...
    real(r8), pointer :: qflx_qsurfg(:)     ! total surface runoff at gridcell level inc land cover change flux (mm H2O /s)
    real(r8), pointer :: qflx_qdraig(:)     ! total subsurface runoff at gridcell level inc land cover change flux (mm H2O /s)
    ...
    real(r8), pointer :: zwt(:)             ! water tabel depth  (m)
...
!-----------------------------------------------------------------------
	...
	!added by Hongyi Li
    call c2g( lbc, ubc, lbl, ubl, lbg, ubg,                &
              qflx_surf(lbc:ubc), qflx_qsurfg(lbg:ubg), &
              c2l_scale_type= 'urbanf', l2g_scale_type='unity' )

    call c2g( lbc, ubc, lbl, ubl, lbg, ubg,                &
              qflx_drain(lbc:ubc), qflx_qdraig(lbg:ubg), &
              c2l_scale_type= 'urbanf', l2g_scale_type='unity' )
			  
    call c2g( lbc, ubc, lbl, ubl, lbg, ubg,                &
              t_grnd(lbc:ubc), t_grndg(lbg:ubg), &
              c2l_scale_type= 'urbans', l2g_scale_type='unity' )

    call c2g( lbc, ubc, lbl, ubl, lbg, ubg,                &
              zwt(lbc:ubc), zwtg(lbg:ubg), &
              c2l_scale_type= 'urbanh', l2g_scale_type='unity' )
    
	do lvl = -nlevsno+1, nlevgrnd
		call c2g( lbc, ubc, lbl, ubl, lbg, ubg,                &
				  t_soisno(lbc:ubc,lvl), t_soisnog(lbg:ubg,lvl), &
				  c2l_scale_type= 'urbans', l2g_scale_type='unity' )
	enddo
