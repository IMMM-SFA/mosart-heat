...
!-----------------------------------------------------------------------
    ...
    str = &
    '$Id: histFileMod.F90 21819 2010-03-19 21:35:50Z erik $'
    ...
    if (mode == 'define') then
       ...
       do ifld = 1,nflds
          ...
          if (tape(t)%dov2xy) then
             ...
             call ncd_iolocal(varname=trim(varnames(ifld)), dim1name=grlnd, dim2name='levgrnd', &
                  data=histo, ncid=nfid(t), flag='write', usepio=pioflag)
          else
             call ncd_iolocal(varname=trim(varnames(ifld)), dim1name=namec, dim2name='levgrnd', &
...
#if (defined RTM)
       call ncd_ioglobal(varname='lonrof', data=rtmCTL%rlon, ncid=nfid(t), flag='write', usepio=pioflag)
       call ncd_ioglobal(varname='latrof', data=rtmCTL%rlat, ncid=nfid(t), flag='write', usepio=pioflag)
#endif
